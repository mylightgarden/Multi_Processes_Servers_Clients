...
int main(int argc, char *argv[]) {
  ...
  // Check usage & args
  if (argc < 4) { 
    fprintf(stderr,"USAGE: dec_client $plaintext $key $port\n"); 
    exit(0); 
  } 
  ...
  // Check if any bad character
   ...
   if (checkResult != 0){
    //error("CLIENT: there are bad character in either plain text or key file.");
  ...
  // To make sure server knows it's from dec_client, send "DEC_CLIENT" to server
  ...
  while (charsRead < plainTextLen){
    ...
    charsRead += curr_read;
    // Append the received content to the cipertext
    printf("%s", buffer);
  }
  // Close the socket
  close(socketFD);
